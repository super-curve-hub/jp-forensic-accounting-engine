...
def load_financial_data(path):
    ...
    numeric_cols = [
        c
        for c in df.columns
        if c not in ["ticker", "company", "sector", "date"]
    ]
...
def regime_label(latest):
    ...
    if pd.notna(roic) and roic > 0.15 and pd.notna(pbr) and pbr < 1:
        return "TSE Reform Candidate"

    if pd.notna(roic) and roic > 0.15 and pd.notna(risk) and risk <= 40:
        return "High ROIC / Monitor"

    return "Neutral / Inconclusive"


def run_jp_forensic_engine(financial_df, ticker, wacc=0.08):
    ...
    latest.update(
        {
            ...
            "Sector": latest.get("sector", "NA"),
            "Grade": forensic_grade(
                latest.get("ForensicRiskScore", np.nan)
            )
        }
    )
